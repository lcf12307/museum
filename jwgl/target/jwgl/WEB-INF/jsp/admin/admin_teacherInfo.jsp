    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="input-group">
                <input id="teacher-input" type="text" class="form-control" placeholder="请输入教师工号">
                <span class="input-group-btn">
                    <button class="btn btn-default sub-btn" type="button">查询</button>
                </span>
            </div>
            <div id="content-div">
...
<script>
    $(document).ready(function(){
        $('.sub-btn').on('click', function () {
            ...
            var url = "http://localhost:8080/admin/teachercourseinfo";
            $.ajax({
                type: 'POST',
                url: url,
                ...
                success: function (jsonResult) {
                    ...
                    $('#content-div').empty();
                    $('#content-div').append(
                        '<table class="table table-hover res-table"><thead><tr><th>课程代号</th><th>课程名称</th><th>上课星期</th><th>开始节次</th><th>结束节次</th><th>开始周</th><th>结束周</th><th>教室</th></tr></thead></table>'
                    );
                    for (var index in jsonResult["result"]) {
                        $('.res-table').append(
                            ...
                            '<td>'+jsonResult["result"][index]["day"]+'</td>'+
                            '<td>'+jsonResult["result"][index]["startTime"]+'</td>'+
                            '<td>'+jsonResult["result"][index]["endTime"]+'</td>'+
                            '<td>'+jsonResult["result"][index]["startWeek"]+'</td>'+
                            '<td>'+jsonResult["result"][index]["endWeek"]+'</td>'+
                            '<td>'+jsonResult["result"][index]["classroom"]+'</td>'+
                            +'<tr>'
                        );
                    }
                },
